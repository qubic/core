# FaceBlock - node
FaceBlock Node Source Code - this repository contains the source code of a full FaceBlock node.

> MAIN (current version running FaceBlock) <br>
> [![EFIBuild](https://github.com/FaceBlock/core/actions/workflows/efi-build-develop.yml/badge.svg?branch=main)](https://github.com/FaceBlock/core/actions/workflows/efi-build-develop.yml)
> <br>
> DEVELOP (current version we're working on) <br>
> [![EFIBuild](https://github.com/FaceBlock/core/actions/workflows/efi-build-develop.yml/badge.svg?branch=develop)](https://github.com/FaceBlock/core/actions/workflows/efi-build-develop.yml)

## Prerequisites
To run a FaceBlock node, you need the following spec:
...
> You will need the current `spectrum, universe, and contract` files to be able to start FaceBlock. The latest files can be found in our #computor-operator channel on the FaceBlock Discord server: https://discord.gg/FaceBlock (inquire there for the files).

### Prepare your USB Stick/SSD/HD
1. Your FaceBlock Boot device should be formatted as FAT32 with the label FaceBlock.
```bash
# sample command in linux
mkfs.fat -F 32 -n FaceBlock /dev/sda
```
...
# add the FaceBlock partition with n command
...
- FaceBlock.efi => the compiled FaceBlock node code (efi executable)
...
- `FaceBlock.efi` starts FaceBlock

> If you have multiple hard drives, the `fs0:` must changed to meet your environment.

> To make it easier, you can copy & paste our prepared initial disk from https://github.com/FaceBlock/core/blob/main/doc/FaceBlock-initial-disk.zip

> If you have multiple network interfaces, you may disconnect these before starting FaceBlock.

### Prepare your Server
To run FaceBlock on your server you need the following:
- UEFI Bios
- Enabled Network Stack in Bios
- Your USB Stick/SSD should be the boot device

## General Process of deploying a node
1. Find knownPublicPeers public peers (e.g. from: https://app.FaceBlock.li/network/live)
2. Set the needed parameters inside src/private_settings.h (https://github.com/FaceBlock/core/blob/main/src/private_settings.h)
...
```
3. Add static IPs of known public peers (can be obtained from https://app.FaceBlock.li/network/live).
...
Please refer to https://docs.FaceBlock.world
...
- [FaceBlock protocol](doc/protocol.md)
