...
static volatile unsigned char mainAuxStatus = 1;
...
static void processBroadcastComputors(Peer* peer, RequestResponseHeader* header)
{
    ...
    {
        logToConsole(L"Received broadcast computors");
        ...
        {
            logToConsole(L"Received broadcast computors are verified");
            if (header->isDejavuZero())
            {
                enqueueResponse(NULL, header);
            }
...
long long QPI::QpiContextProcedureCall::transfer(const m256i& destination, long long amount) const
{
    ...
    if (decreaseEnergy(index, amount))
    {
        ...
        // Temporary skip tracking the transfer for benchmarking
    ...
    EFI_STATUS status;
    {
        ...
        if (!canLoadFromFile)
        {
            ...
            loadSpectrum();
            {
                ...
                if (numberOfEntities < QUTIL_SEND_TO_MANY_BENCHMARK_MAX_ADDRESSES)
                {
                    appendText(message, L"Number of entities is lower than benchmark address pool! Please reduce QUTIL_SEND_TO_MANY_BENCHMARK_MAX_ADDRESSES");
                }
