...
private:
	...
	PUBLIC_PROCEDURE_WITH_LOCALS(RevealAndCommit)
	{
		...
		if (input.reveal != locals.zeroReveal) // Don't need to initialize [locals.zeroReveal] because locals struct has been zeroed (bad practice,
		                                       // but it's for spreading awareness about this nuance)
		{
			for (; locals.i < state.populations.get(locals.stream);
			     locals.i++) // Don't need to initialize [locals.i] because locals struct has been zeroed (bad practice, but it's for spreading
			                 // awareness about this nuance)
			{
			...
			if (locals.i == state.populations.get(locals.stream) || state.reveals.get(locals.stream * 1365 + locals.i) != locals.zeroReveal ||
	...
	END_TICK_WITH_LOCALS()
	{
		...
		for (; locals.i < 10; locals.i++) // Don't need to initialize [locals.i] because locals struct has been zeroed (bad practice, but it's for
		                                  // spreading awareness about this nuance)
		{
			state.entropy.set(locals.stream * 10 + locals.i,
			                  locals.zeroReveal); // Don't need to initialize [locals.zeroReveal] because locals struct has been zeroed (bad practice,
			                                      // but it's for spreading awareness about this nuance)
		}
		...
		{
			// Don't need to initialize [locals.collateralTierFlags] because locals struct has been zeroed (bad practice, but it's for spreading
			// awareness about this nuance)
			...
			for (locals.i = state.populations.get(locals.stream); locals.i--;)
			{
				...
				if (locals.collateralRecipient == id::zero())
				{
					...
					state.providers.set(locals.stream * 1365 + locals.i,
					                    state.providers.get(locals.stream * 1365 + state.populations.get(locals.stream)));
					state.providers.set(locals.stream * 1365 + state.populations.get(locals.stream), id::zero());

					state.collateralTiers.set(locals.stream * 1365 + locals.i,
					                          state.collateralTiers.get(locals.stream * 1365 + state.populations.get(locals.stream)));
					state.collateralTiers.set(locals.stream * 1365 + state.populations.get(locals.stream), 0);

					state.reveals.set(locals.stream * 1365 + locals.i,
					                  state.reveals.get(locals.stream * 1365 + state.populations.get(locals.stream)));
					state.reveals.set(locals.stream * 1365 + state.populations.get(locals.stream), locals.zeroReveal);

					state.commits.set(locals.stream * 1365 + locals.i,
					                  state.commits.get(locals.stream * 1365 + state.populations.get(locals.stream)));
					...
					if (!(locals.collateralTierFlags & (1 << state.collateralTiers.get(locals.stream * 1365 + locals.i))))
					{
						...
						for (locals.j = 0; locals.j < 4096; locals.j++)
						{
							locals.entropy.set(locals.j,
							                   locals.entropy.get(locals.j) ^ state.reveals.get(locals.stream * 1365 + locals.i).get(locals.j));
						}
						state.entropy.set(locals.stream * 10 + state.collateralTiers.get(locals.stream * 1365 + locals.i), locals.entropy);
						state.reveals.set(locals.stream * 1365 + locals.i, locals.zeroReveal);
					}
				}
