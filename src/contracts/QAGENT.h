...
{
    TaskRecord task;
    AIVerifyOracleQuery oracleQuery;
    QAGENTLogger log;
};

// Using-alias instead of typedef to satisfy contract-verify while preserving
// OracleNotificationInput<AIVerifyOracle> type identity for QUERY_ORACLE template deduction.
// See qubic-contract-verify#4 for the underlying verifier limitation.
using OnAIVerifyReply_input = OracleNotificationInput<AIVerifyOracle>;
...
PUBLIC_PROCEDURE_WITH_LOCALS(RequestOracleVerification)
{
    ...
    output.oracleQueryId = QUERY_ORACLE(AIVerifyOracle, locals.oracleQuery, OnAIVerifyReply, QAGENT_ORACLE_TIMEOUT_MS);
