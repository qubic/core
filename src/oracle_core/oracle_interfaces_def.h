...
#include "oracle_interfaces/AIVerify.h"
#undef ORACLE_INTERFACE_INDEX
...
#define DEFINE_ORACLE_INTERFACE(Interface) {sizeof(Interface::OracleQuery), sizeof(Interface::OracleReply)}
	...
	constexpr struct {
		...
		DEFINE_ORACLE_INTERFACE(AIVerify),
...
#define REGISTER_ORACLE_INTERFACE(Interface) { \
	...
	static bool initOracleInterfaces()
	{
		...
		REGISTER_ORACLE_INTERFACE(AIVerify);
...
// Type aliases for contract use (avoids OI:: scope in contract files)
using AIVerifyOracle = OI::AIVerify;
using AIVerifyOracleQuery = OI::AIVerify::OracleQuery;
