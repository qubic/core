...
TEST(TestAntPending, DedupsOnTheConsensusKey)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);

    EXPECT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));
    EXPECT_FALSE(pool->add(key(1), ref(100, 0), 5000, key(900)));
    ...
    EXPECT_FALSE(pool->add(key(1), ref(100, 0), 9999, key(900)));

    // Any other field differing makes it a different solution.
    EXPECT_TRUE(pool->add(key(2), ref(100, 0), 5000, key(900)));
    EXPECT_TRUE(pool->add(key(1), ref(100, 1), 5000, key(900)));
    EXPECT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(901)));
}

// A fresh entry is selectable, and only by the computor it belongs to.
TEST(TestAntPending, SelectsOnlyForItsOwnComputor)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ASSERT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));

    AntPendingSolution out;
    EXPECT_EQ(pool->selectForPublish(key(2), 5000, out), NO_ENTRY);

    const unsigned int idx = pool->selectForPublish(key(1), 5000, out);
    ASSERT_NE(idx, NO_ENTRY);
...
TEST(TestAntPending, ScheduledEntryIsNotReselectedBeforeItsDeadline)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ASSERT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));
    ...
    EXPECT_EQ(pool->selectForPublish(key(1), 5001, out), NO_ENTRY);
    EXPECT_EQ(pool->selectForPublish(key(1), 5002, out), NO_ENTRY);
...
TEST(TestAntPending, RetriesOutrankFreshEntries)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ...
    ASSERT_NE(stale, NO_ENTRY);
    pool->markScheduled(stale, 5003);

    // Newer solutions keep arriving while the first one's transaction is lost.
    ASSERT_TRUE(pool->add(key(1), ref(100, 1), 5001, key(901)));
    ASSERT_TRUE(pool->add(key(1), ref(100, 2), 5002, key(902)));
...
TEST(TestAntPending, RecordedStopsRepublishingAndSuppressesResend)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ASSERT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));
    ...
    EXPECT_EQ(pool->selectForPublish(key(1), 9000, out), NO_ENTRY);
    EXPECT_FALSE(pool->add(key(1), ref(100, 0), 5000, key(900)));
}
...
TEST(TestAntPending, RecordingAnUnqueuedSolutionSuppressesALaterSubmission)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
...
TEST(TestAntPending, ExpiredEntriesAreRetiredNotPublished)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ASSERT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));
    ...
    EXPECT_EQ(pool->selectForPublish(key(1), 5000 + ANT_PUBLISH_WINDOW_TICKS + 1, out), NO_ENTRY);
...
TEST(TestAntPending, ExpiredEntryCanBeResubmittedWithAFreshAnchor)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ASSERT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));
    ...
    AntPendingSolution out;
    ASSERT_EQ(pool->selectForPublish(key(1), 5000 + ANT_PUBLISH_WINDOW_TICKS + 1, out), NO_ENTRY);

    // Same triple, newer anchor. This is the replacement, not a duplicate.
    EXPECT_TRUE(pool->add(key(1), ref(100, 0), 40000, key(900)));
...
TEST(TestAntPending, LiveEntryStillRejectsAResubmission)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ASSERT_TRUE(pool->add(key(1), ref(100, 0), 5000, key(900)));

    EXPECT_FALSE(pool->add(key(1), ref(100, 0), 6000, key(900)));
...
TEST(TestAntPending, FinishedSlotsAreReclaimed)
{
    AntPendingSolutions* pool = freshPool();
    ASSERT_NE(pool, nullptr);
    ...
    {
        ASSERT_TRUE(pool->add(key(1), ref(100, i), 5000, key(900 + i)));
    ...
    AntPendingSolution out;
    EXPECT_EQ(pool->selectForPublish(key(1), 5000, out), NO_ENTRY);
    EXPECT_TRUE(pool->add(key(1), ref(200, 0), 5000, key(1000)));
}
