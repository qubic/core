...
>
struct ScoreReferenceImplementationQiner
{
    ...
    ~ScoreReferenceImplementationQiner()
    {
        freeMemory();
    }
    ...
    unsigned int operator()(unsigned long long processorNumber, unsigned char* publicKey, unsigned char* nonce)
    {
        ...
        memset(neurons->input, 0, sizeof(Neuron));

        random2(publicKey, nonce, (unsigned char*)synapses, sizeof(Synapse), random2PoolBuffer);
        memcpy(&neurons->input, miningData, sizeof(miningData));
