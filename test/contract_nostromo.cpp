...
public:
    ...
    void totalRaisedFundChecker(uint32 indexOfFundaraising, uint64 raisedFund)
    {
        ...
        {
            Asset assetInfo;
            assetInfo.assetName = assetNameFromString("AAAA");
            ...
            EXPECT_EQ(numberOfPossessedShares(assetNameFromString("AAAA"), id(NOST_CONTRACT_INDEX, 0, 0, 0), projects.get(fundaraisings.get(indexOfFundaraising).indexOfProject).creator, projects.get(fundaraisings.get(indexOfFundaraising).indexOfProject).creator, 13, 13), projects.get(fundaraisings.get(indexOfFundaraising).indexOfProject).supplyOfToken - fundaraisings.get(indexOfFundaraising).soldAmount);
        }
    }
};
...
TEST(TestContractNostromo, createFundaraisingAndInvestInProjectAndClaimTokenChecker)
{
    ...
    nostromo.registerInTier(registers[0], 5, NOSTROMO_TIER_WARRIOR_STAKE_AMOUNT);
    uint64 assetName = assetNameFromString("AAAA");
    nostromo.createProject(registers[0], assetName, 21000000, 25, 6, 13, 0, 25, 6, 15, 0);
    ...
    */
    ...
    for (const auto& user : registers)
    {
        ...
        nostromo.voteInProject(user, 0, decision);
        duplicatedUser[user] = 1;
    }
    nostromo.getState()->voteInProjectChecker(0, Ynumber, Nnumber);
    ...
    */
    ...
    nostromo.createFundaraising(registers[0], 100, 2000000, 150000000, 0, 
        25, 6, 17, 0,
        25, 6, 25, 0,
        25, 6, 28, 0,
        25, 7, 1, 0,
    ...
    nostromo.getState()->countOfFundaraisingChecker(0);
    ...
    nostromo.createFundaraising(registers[0], 100000, 2000000, 150000000000, 0, 
        25, 6, 17, 0,
        25, 6, 25, 0,
        25, 6, 28, 0,
        25, 7, 1, 0,
    ...
    nostromo.getState()->countOfFundaraisingChecker(1);
    nostromo.getState()->createFundaraisingChecker(registers[0], 100000, 2000000, 150000000000, 0, 
        25, 6, 17, 0,
        25, 6, 25, 0,
        25, 6, 28, 0,
        25, 7, 1, 0,
    ...
    */
    ...
    for (const auto& user : registers)
    {
        ...
        uint32 tierLevel = nostromo.getState()->getTierLevel(user);
        ...
        case 1:
            ...
            nostromo.investInProject(user, 0, facehuggerMaxInvestAmount + overDeposit);
            break;
        case 2:
            if (ct < 4000)
            {
            ...
            nostromo.investInProject(user, 0, chestburstMaxInvestAmount + overDeposit);
            break;
        case 3:
            if (ct < 4000)
            {
            ...
            nostromo.investInProject(user, 0, dogMaxInvestAmount + overDeposit);
            break;
        case 4:
            totalInvestedAmount += xenomorphMaxInvestAmount;
            nostromo.investInProject(user, 0, xenomorphMaxInvestAmount + overDeposit);
            break;
        case 5:
            totalInvestedAmount += warriorMaxInvestAmount;
            nostromo.investInProject(user, 0, warriorMaxInvestAmount + overDeposit);
            break;
    ...
    */
    ...
    for (const auto& user : registers)
    {
        ...
        nostromo.investInProject(user, 0, amount);
        
        duplicatedUser[user] = 1;
    }

    nostromo.getState()->totalRaisedFundChecker(0, totalInvestedAmount);
    ...
    for (uint32 i = 1; i <= 12; i++)
    {
        ...
        for (const auto& user : registers)
        {
            ...
            uint64 investedAmount = nostromo.getState()->getInvestedAmount(0, user);
            uint64 claimAmount = investedAmount / 100000 / 12;

            nostromo.claimToken(user, claimAmount, 0);
    ...
    for (const auto& user : registers)
    {
        ...
        if (ct == 0)
        {
            EXPECT_EQ(numberOfPossessedShares(assetName, id(NOST_CONTRACT_INDEX, 0, 0, 0), user, user, NOST_CONTRACT_INDEX, NOST_CONTRACT_INDEX) - 19000000, nostromo.getState()->getInvestedAmount(0, user) / 1200000 * 12);
        }
        else
        {
            EXPECT_EQ(numberOfPossessedShares(assetName, id(NOST_CONTRACT_INDEX, 0, 0, 0), user, user, NOST_CONTRACT_INDEX, NOST_CONTRACT_INDEX), nostromo.getState()->getInvestedAmount(0, user) / 1200000 * 12);
        }
        ct++;
        duplicatedUser[user] = 1;
    }
    ...
    */
    ...
    Asset asset;
    asset.assetName = assetNameFromString("AAAA");
    asset.issuer = id(NOST_CONTRACT_INDEX, 0, 0, 0);
    EXPECT_EQ(nostromo.TransferShareManagementRights(registers[0], asset, 10000, QX_CONTRACT_INDEX), 10000);
