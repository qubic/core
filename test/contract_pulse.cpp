...
namespace
{
    ...
    static void primeQpiProcedureContext(QpiContextUserProcedureCall& qpi)
    {
    ...
    static Array<uint8, PULSE_PLAYER_DIGITS + 2> makePlayerDigits(
        uint8 d0, uint8 d1, uint8 d2, uint8 d3, uint8 d4, uint8 d5)
    {
    ...
    static void expectWinningDigitsUniqueAndInRange(const Array<uint8, PULSE_WINNING_DIGITS + 7>& digits)
    {
        std::set<uint8> seen;
...
public:
    ...
    ValidateDigits_output callValidateDigits(
        const QPI::QpiContextFunctionCall& qpi,
        const Array<uint8, PULSE_PLAYER_DIGITS + 2>& digits) const
    {
...
public:
    ...
    {
        char name[7] = { 'Q', 'H', 'E', 'A', 'R', 'T', 0 };
        char unit[7] = {};
        QHeartIssuance info{};
        const long long issued = issueAsset(PULSE_QHEART_ISSUER, name, 0, unit, totalShares,
            PULSE_CONTRACT_INDEX, &info.issuanceIndex, &info.ownershipIndex, &info.possessionIndex);
    ...
    void transferQHeart(const QHeartIssuance& issuance, const id& dest, sint64 amount)
    {
        ...
        EXPECT_TRUE(transferShareOwnershipAndPossession(
            issuance.ownershipIndex, issuance.possessionIndex, dest, amount,
            &destOwnershipIndex, &destPossessionIndex, true));
    }
    ...
    {
        const long long balance = numberOfPossessedShares(
            PULSE_QHEART_ASSET_NAME, PULSE_QHEART_ISSUER, owner, owner,
            PULSE_CONTRACT_INDEX, PULSE_CONTRACT_INDEX);
...
TEST(ContractPulse_Private, ValidateDigitsRejectsDuplicateAndOutOfRange)
{
    ...
    const auto ok = makePlayerDigits(0, 1, 2, 3, 4, 5);
    EXPECT_TRUE(ctl.state()->callValidateDigits(qpi, ok).isValid);

    const auto dup = makePlayerDigits(0, 1, 2, 3, 4, 4);
    EXPECT_FALSE(ctl.state()->callValidateDigits(qpi, dup).isValid);

    const auto outOfRange = makePlayerDigits(0, 1, 2, 3, 4, 10);
...
TEST(ContractPulse_Private, GetRandomDigitsDeterministicAndUnique)
{
    ...
    const auto out1 = ctl.state()->callGetRandomDigits(qpi, seed);
    const auto out2 = ctl.state()->callGetRandomDigits(qpi, seed);
...
{
    ContractTestingPulse ctl;
    const auto issuance = ctl.issueQHeart(1000000);
    ...
    const auto winning = ctl.state()->callGetRandomDigits(qpiFunc, seed).digits;

    const id player = id::randomValue();
    const auto ticketDigits = makePlayerDigits(
        winning.get(0), winning.get(1), winning.get(2),
        winning.get(3), winning.get(4), winning.get(5));
...
TEST(ContractPulse_Public, GettersReturnDefaultsAfterInitialize)
{
    ...
    const auto win = ctl.getWinningDigits();
    for (uint64 i = 0; i < PULSE_WINNING_DIGITS; ++i)
    {
...
TEST(ContractPulse_Public, SetPriceGuardsAccessAndAppliesOnEndEpoch)
{
    ContractTestingPulse ctl;
    EXPECT_EQ(
        ctl.setPrice(id::randomValue(), 123).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::ACCESS_DENIED));
    EXPECT_EQ(
        ctl.setPrice(PULSE_QHEART_ISSUER, 0).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::TICKET_INVALID_PRICE));

    EXPECT_EQ(
        ctl.setPrice(PULSE_QHEART_ISSUER, 555).returnCode,
...
TEST(ContractPulse_Public, SetScheduleValidatesAndAppliesOnEndEpoch)
{
    ContractTestingPulse ctl;
    EXPECT_EQ(
        ctl.setSchedule(id::randomValue(), 1).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::ACCESS_DENIED));
    EXPECT_EQ(
        ctl.setSchedule(PULSE_QHEART_ISSUER, 0).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::INVALID_VALUE));

    EXPECT_EQ(
        ctl.setSchedule(PULSE_QHEART_ISSUER, 0x7F).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::SUCCESS));
    ctl.endEpoch();
...
TEST(ContractPulse_Public, SetDrawHourValidatesAndAppliesOnEndEpoch)
{
    ContractTestingPulse ctl;
    EXPECT_EQ(
        ctl.setDrawHour(id::randomValue(), 12).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::ACCESS_DENIED));
    EXPECT_EQ(
        ctl.setDrawHour(PULSE_QHEART_ISSUER, 24).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::INVALID_VALUE));

    EXPECT_EQ(
        ctl.setDrawHour(PULSE_QHEART_ISSUER, 9).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::SUCCESS));
    ctl.endEpoch();
...
TEST(ContractPulse_Public, SetFeesValidatesAndAppliesOnEndEpoch)
{
    ContractTestingPulse ctl;
    EXPECT_EQ(
        ctl.setFees(id::randomValue(), 1, 2, 3, 4).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::ACCESS_DENIED));
    EXPECT_EQ(
        ctl.setFees(PULSE_QHEART_ISSUER, 60, 60, 0, 0).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::INVALID_VALUE));

    EXPECT_EQ(
        ctl.setFees(PULSE_QHEART_ISSUER, 11, 22, 33, 4).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::SUCCESS));
    ctl.endEpoch();
    EXPECT_EQ(ctl.state()->getDevPercentInternal(), 11u);
    EXPECT_EQ(ctl.state()->getBurnPercentInternal(), 22u);
...
TEST(ContractPulse_Public, SetQHeartHoldLimitAppliesOnEndEpoch)
{
    ContractTestingPulse ctl;
    EXPECT_EQ(
        ctl.setQHeartHoldLimit(id::randomValue(), 100).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::ACCESS_DENIED));
    EXPECT_EQ(
        ctl.setQHeartHoldLimit(PULSE_QHEART_ISSUER, 1234).returnCode,
        static_cast<uint8>(PULSE::EReturnCode::SUCCESS));
    ctl.endEpoch();
...
{
    ContractTestingPulse ctl;
    const auto out = ctl.buyTicket(id::randomValue(), makePlayerDigits(0, 1, 2, 3, 4, 5));
...
TEST(ContractPulse_Public, BuyTicketValidatesDigits)
{
    ...
    const auto out = ctl.buyTicket(user, makePlayerDigits(0, 1, 2, 3, 4, 4));
...
TEST(ContractPulse_Public, BuyTicketFailsWithInsufficientBalance)
{
    ...
    const auto out = ctl.buyTicket(user, makePlayerDigits(0, 1, 2, 3, 4, 5));
...
TEST(ContractPulse_System, PostIncomingTransferStandardTransactionReturnsFunds)
{
    ContractTestingPulse ctl;
    const id sender = id::randomValue();
    const uint64 amount = 5000;
    increaseEnergy(ctl.pulseSelf(), amount);
    const uint64 senderBefore = getBalance(sender);

    QpiContextSystemProcedureCall qpi(PULSE_CONTRACT_INDEX, POST_INCOMING_TRANSFER);
    QPI::PostIncomingTransfer_input input{ sender, static_cast<sint64>(amount), QPI::TransferType::standardTransaction };
    qpi.call(input);

    EXPECT_EQ(getBalance(sender), senderBefore + amount);
}
